...
signal wordle_completed(success: bool)
...
func get_random_wordle():
	var file = FileAccess.open("res://scenes/minigames/wordle/wordle-list.txt", FileAccess.READ)
...
func check_win():
	...
	if entered_text == wordle:
		...
		menu.show()
		emit_signal("wordle_completed", true)
		queue_free() # <-- FECHA O MINIGAME
		return
	...
	if index == buttons.size():
		...
		menu.show()
		emit_signal("wordle_completed", false)
		queue_free() # <-- FECHA O MINIGAME
